...
function formatTime(seconds) {
    ...
    const s = seconds % 60;
    return `${String(m).padStart(2, '0')}:${String(s).padStart(5, '0').replace(' ', '0')}`;
}
...
async function play(reqIn) {
    ...
    let lrc = '';
    try {
        const lrcApi = `https://kuwo.cn/openapi/v1/www/lyric/getlyric?musicId=${rid}`;
        const lrcData = await getJson(lrcApi);
        if (lrcData.data && lrcData.data.lrclist) {
            const lines = [];
            for (const item of lrcData.data.lrclist) {
